...
$ エントリが TMAX_MAXCYC(kernel.h) 個より多い場合は，エラーとする
$IF LENGTH(CYC.ORDER_LIST) > TMAX_MAXCYC$
	$ERROR$$FORMAT(_("The number of CRE_CYC must be equal to or less than %1%."), +VALUE("TMAX_MAXCYC", TMAX_MAXCYC))$$END$
$END$
...
$ エントリが TMAX_MAXALM(kernel.h) 個より多い場合は，エラーとする
$IF LENGTH(ALM.ORDER_LIST) > TMAX_MAXALM$
	$ERROR$$FORMAT(_("The number of CRE_ALM must be equal to or less than %1%."), +VALUE("TMAX_MAXALM",TMAX_MAXALM))$$END$
$END$
